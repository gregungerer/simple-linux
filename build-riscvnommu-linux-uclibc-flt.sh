...
#
# build-riscvnommu-linux-uclibc-flt.sh -- build really simple nommu linux for RISC-V
#
...
#		-nographic \
#		-machine virt \
#		-bios linux-6.11/arch/riscv/boot/Image
#

CPU=riscv
TARGET=riscv64-linux-uclibc
FLAVOR=riscvnommu-flt
BOARD=qemu
...
build_busybox()
{
	...
	sed -i 's/CONFIG_EXTRA_LDFLAGS=""/CONFIG_EXTRA_LDFLAGS="-Wl,-elf2flt=-r"/' .config
...
if [ "$#" != 0 ]
then
	echo "usage: build-riscvnommu-linux-uclibc-flt.sh [clean]"
	exit 1
fi
