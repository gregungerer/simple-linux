...
#
# build-m68k-linux-uclibc-elf.sh -- build really simple linux for m68k
#
# (C) Copyright 2022-2026, Greg Ungerer (gerg@kernel.org)
#
# This script carries out a simple build of an m68k based user space
# and linux for use with the MAC q800 qemu emulated machine.
#
...
#
#  qemu-system-m68k -machine q800 -kernel linux-6.18/vmlinux
#
...
if [ "$#" != 0 ]
then
	echo "usage: build-m68k-linux-uclibc-elf.sh [clean]"
	exit 1
fi
