...
#
# build-sh-linux-uclibc-elf.sh -- build really simple linux for SuperH
#
# (C) Copyright 2022-2026, Greg Ungerer (gerg@kernel.org)
#
# This script carries out a simple build of a SuperH based user space
# and linux for use with the sh/r2d qemu emulated machine.
#
...
#		-kernel linux-6.19/arch/sh/boot/zImage
#

CPU=sh
TARGET=sh-linux-uclibc
FLAVOR=sh-elf
BOARD=rts7751r2dplus
...
build_busybox()
{
	...
	sed -i 's/CONFIG_EXTRA_CFLAGS=""/CONFIG_EXTRA_CFLAGS="-m4 -ml"/' .config
...
if [ "$#" != 0 ]
then
	echo "usage: build-sh-linux-uclibc-elf.sh [clean]"
	exit 1
fi
