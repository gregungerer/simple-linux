...
build_busybox()
{
	...
	sed -i 's/CONFIG_EXTRA_CFLAGS=""/CONFIG_EXTRA_CFLAGS="-mthumb -march=armv7-m"/' .config
...
#
# The QEMU emulation of Corext-M doesn't have any of the nice kernel or
# DTB loading options that strait ARM has. We essentially need a small
# boot loader as well to boot strap from the initial power up vectors
# into the kernel proper. I guess u-boot would probably be the best option,
# but we can use an ARM created simple boot wrapper of the kernel to do
# the loading for us. Need to tweak it a little to load the MPS2-AN385
# platform image.
#
build_bootwrapper()
{
	...
	cp ../linux-${LINUX_VERSION}/arch/arm/boot/dts/arm/mps2-an385.dtb .
	sed -i 's/mps2-an399.dtb/mps2-an385.dtb/' Makefile
...
if [ "$#" != 0 ]
then
	echo "usage: build-cortexm-linux-uclibc-flt.sh [clean]"
	exit 1
fi
